...
router.post('/', async (req,res) => {
    ...
    try {
        ...
        res.status(201).json(newUser); // automaticlly sends 200 which means successfull but 201 means you created something successfull
    } catch(err) {
...
router.patch('/:id', getUser, async (req,res) => {
    ...
    if (req.body.typeOfUser != null){
        res.article.typeOfUser = req.body.typeOfUser
    }
    try {
...
async function getUser(req, res, next){
    ...
    try {
        ...
        if (user == null) return res.status(404).json({message: 'Cannot find User.'}) // 404 = could not find something, Article
    } catch (err) {
