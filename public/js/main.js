...
function getComments(articleID, fbPostID) {
  ...
  updateCommentsBtn.addEventListener("click", function () {
    getArticleComments(fbPostID, articleID); //fb.js
  });
...
function createCommentsMarkup(response, articleID) {
  let commentDiv = document.getElementById(articleID).getElementsByClassName('getFBComments')[0];
  ...
  for(let i = 0; i < res.length; i++){
    ...
    oneComment.setAttribute("id", res[i]._id);
